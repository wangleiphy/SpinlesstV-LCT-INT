...
data = pyalps.loadMeasurements(resultFiles, 'M2')
M2 = pyalps.collectXY(data, x='V', y='M2', foreach = ['L'])
pyalps.propsort(M2, 'L')
print 'M2 done'
...
for d in data:
    ...
    #r.y = [abs(d.y[L])]
    r.props = d.props 
    r.props['observable'] = 'CRmax'

    res.append(r)

CRmax = pyalps.collectXY(res, x='V', y='CRmax', foreach = ['L'])
pyalps.propsort(CRmax,'L')

res = []
for d1, d2 in zip(M2, CRmax):
    assert(d1.props['L']==d2.props['L'])
    if (len(d1.x) != len(d2.x)): continue
    ...
    r.props['observable'] = 'Binder'
    r.props['ylabel'] = r'$M_2/L^{2}/C(R_\mathrm{max})$'
    r.props['line'] = '-o'
...
if args.Scaling:
    for d in res:
        ...
        d.props['xlabel'] = r'$(U-U_c)L^{a}$'
        d.props['ylabel'] = r'$S_\mathrm{AF}/L^{2}L^{b}$'
        d.props['label'] = '$L=%g$' %(L)
        d.props['line'] = '-o'
...
if args.Scaling and args.xc!=0.0:
